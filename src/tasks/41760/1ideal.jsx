...
const TaskCard = ({ task, onTaskClick, isActive }) => {
  ...
  return (
     // Card component with dynamic styling based on whether the task is active
    <Card
...
const Timer = ({ workInterval, breakInterval, onTimerEnd }) => {
  ...
  // Effect to handle timer countdown and transitions
  useEffect(() => {
    let timer;
    if (isRunning && timeLeft > 0) {
      ...
      // Switch between work and break intervals when time runs out
...
export default function App() {
  ...
  const startTask = (task) => {
    if (activeTask) {
      return; // If there is already an active task, do nothing
    }
  ...
  const handleTimerEnd = () => {
    if (activeTask) {
      // Update the status of the active task based on current status
      const updatedTasks = tasks.map((t) =>
        t.id === activeTask.id
          ? {
              ...t,

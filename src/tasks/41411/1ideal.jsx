...
const StatusDropdown = ({ onStatusChange }) => {
  ...
  // Find the color class for the selected status
  const selectedOption = statusOptions.find(option => option.value === selectedStatus);
  const selectedColor = selectedOption ? selectedOption.color : '';

  return (
    <Select value={selectedStatus} onValueChange={handleStatusChange}>
      <SelectTrigger className={`w-full sm:w-64 ${selectedColor}`}>

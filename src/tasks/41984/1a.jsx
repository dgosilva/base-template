...
export default function App() {
  ...
  const handleSendMessage = () => {
    ...
    const newMessage = { id: Date.now(), text: message, sent: true, pending: true };
      ...
      setTimeout(() => {
        setSelectedChat((prev) => ({
          ...
          messages: prev.messages.map((m) =>
            m.id === newMessage.id ? { ...m, error: true } : m
          ),
        }));
      }, 1000);
      return;
    ...
    api.sendMessage(selectedChat.id, message).then((sentMessage) => {
      setSelectedChat((prev) => ({
        ...prev,
        messages: prev.messages.map((m) =>
          m.id === newMessage.id ? { ...sentMessage, pending: false } : m
        ),
      }));
  ...
  const handleRetry = (messageId) => {
    setSelectedChat((prev) => ({
      ...prev,
      messages: prev.messages.map((m) =>
        m.id === messageId ? { ...m, error: false, pending: true } : m
      ),
    }));
    handleSendMessage();
  };
